...
export async function hrLogin(data: LoginDataInterface) {
  try {
    const res = await axios.post<HrResponseInterface>(config.HR_BE_BASE_URL + '/employee/login', data);
    return res.data;
  } catch (error) {
...
export async function hrServiceCheck(data: { userId: number; service: string }) {
  try {
    const res = await axios.post<{ status: string; auth: boolean }>(config.HR_BE_BASE_URL + "/access/check", data);
    return res.data;
  } catch (error) {
    console.log(error);
    throw new Error((error as AxiosError<{ message: string }>).response?.data.message);
...
export async function hrServiceList(userId: number) {
  try {
    const res = await axios.get<{ services: string[] }>(config.HR_BE_BASE_URL + "/access/service/" + userId);
    return res.data;
  } catch (error) {
    console.log(error);
    throw new Error((error as AxiosError<{ message: string }>).response?.data.message);
